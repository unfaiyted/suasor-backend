...
type UserCollectionHandler struct {
	...
	collectionService     services.UserCollectionService
}
...
func (h *UserCollectionHandler) GetUser(c *gin.Context) {
	...
	collections, err := h.userCollectionService.GetByUserID(ctx, uid)
	if err != nil {
		log.Error().Err(err).
			Uint64("userID", uid).
...
func (h *UserCollectionHandler) AddItem(c *gin.Context) {
	...
	err = h.collectionService.AddItemToCollection(ctx, id, req.ItemID)
	if err != nil {
		log.Error().Err(err).
			Uint64("collectionID", id).
			Uint64("itemID", req.ItemID).
...
func (h *UserCollectionHandler) RemoveItem(c *gin.Context) {
	...
	err = h.collectionService.RemoveItemFromCollection(ctx, id, itemID)
	if err != nil {
		log.Error().Err(err).
			Uint64("collectionID", id).
			Uint64("itemID", itemID).
