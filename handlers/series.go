...
// SeriesSpecificHandler extends MediaItemHandler with series-specific functionality
type SeriesSpecificHandler struct {
	*MediaItemHandler[*mediatypes.Series]
	episodeHandler *MediaItemHandler[*mediatypes.Episode]
	seasonHandler  *MediaItemHandler[*mediatypes.Season]
}
...
func (h *SeriesSpecificHandler) GetSeasonsBySeriesID(c *gin.Context) {
	...
	series, err := h.service.GetByID(ctx, seriesID)
	if err != nil {
		log.Error().Err(err).
			Uint64("seriesID", seriesID).
...
func (h *SeriesSpecificHandler) GetEpisodesBySeriesIDAndSeasonNumber(c *gin.Context) {
	ctx := c.Request.Context()
	log := utils.LoggerFromContext(ctx)
...
func (h *SeriesSpecificHandler) GetContinueWatchingSeries(c *gin.Context) {
	ctx := c.Request.Context()
	log := utils.LoggerFromContext(ctx)
...
func (h *SeriesSpecificHandler) GetAllEpisodes(c *gin.Context) {
	ctx := c.Request.Context()
	log := utils.LoggerFromContext(ctx)
...
func (h *SeriesSpecificHandler) GetNextUpEpisodes(c *gin.Context) {
	ctx := c.Request.Context()
	log := utils.LoggerFromContext(ctx)
...
func (h *SeriesSpecificHandler) GetRecentlyAiredEpisodes(c *gin.Context) {
	ctx := c.Request.Context()
	log := utils.LoggerFromContext(ctx)
...
func (h *SeriesSpecificHandler) GetSeriesByNetwork(c *gin.Context) {
	ctx := c.Request.Context()
	log := utils.LoggerFromContext(ctx)
	...
	allSeries, err := h.service.GetByUserID(ctx, userID)
	if err != nil {
		log.Error().Err(err).
			Uint64("userID", userID).
			Msg("Failed to retrieve series")
	...
	var result []*mediatypes.Series

	for _, series := range allSeries {
		if series.Data.Network == network {
			filteredSeries = append(filteredSeries, series.Data)
	...
	// If Series type doesn't have a Network field, respond with not implemented
	if len(filteredSeries) == 0 {
		log.Info().Msg("Network-based filtering not fully implemented")
		responses.RespondOK(c, result, "Series retrieved successfully (network field may not be available)")
		return
	}
