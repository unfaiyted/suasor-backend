...
func (s *userCollectionService) Delete(ctx context.Context, id uint64) error {
	...
	err = s.coreService.Delete(ctx, id)
	if err != nil {
		log.Error().Err(err).
			Uint64("id", id).
...
func (s *userCollectionService) GetByUserID(ctx context.Context, userID uint64) ([]*models.MediaItem[*mediatypes.Collection], error) {
	log := utils.LoggerFromContext(ctx)
	log.Debug().
	...
	collections, err := s.userRepo.GetByUserID(ctx, userID)
	if err != nil {
		log.Error().Err(err).
			Uint64("userID", userID).
