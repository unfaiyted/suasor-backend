...
func (c AIClientType) AsGenericClient() ClientType {
	return ClientType(c)
}
...
func GetClientType[T ClientConfig]() ClientType {
	var zero T

	switch config := any(zero).(type) {
	case *EmbyConfig:
		return ClientTypeEmby
	case *JellyfinConfig:
		return ClientTypeJellyfin
	...
	if clientType := zero.GetType(); clientType != ClientTypeUnknown {
		return clientType
	}

	// Last resort: Check type name string
